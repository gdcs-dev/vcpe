...
on commit {
    ...
    set clhn = pick-first-value(option host-name, "");
    execute("/etc/dhcpd-notify.sh", "commit", clip, clhw, clhn);
}
...
on release {
    ...
    set clhw = binary-to-ascii(16, 8, ":", substring(hardware, 1, 6));
    set clhn = pick-first-value(option host-name, "");
    execute("/etc/dhcpd-notify.sh", "release", clip, clhw, clhn);
}
...
on expiry {
    ...
    set clhw = binary-to-ascii(16, 8, ":", substring(hardware, 1, 6));
    set clhn = pick-first-value(option host-name, "");
    execute("/etc/dhcpd-notify.sh", "expiry", clip, clhw, clhn);
}
