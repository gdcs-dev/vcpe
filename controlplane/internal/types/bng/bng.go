...
func decodeConfig(node yaml.Node) (Config, error) {
	...
	for _, segment := range config.Access {
		if segment.Role == "" {
			return Config{}, fmt.Errorf("bng access segment is missing role")
		}
	}
	return config, nil
}
...
func renderBNGInstance(_ context.Context, input render.Input, cfg Config) (render.Result, error) {
	...
	for _, iface := range inst.Interfaces {
		...
		address := render.IPWithPrefix(iface.IPv4, n.IPv4.CIDR)
		if address == iface.IPv4 {
			continue
		}
		roleKey := strings.ToUpper(strings.ReplaceAll(iface.Role, "-", "_"))
		env = append(env, fmt.Sprintf("%s_IPV4_CIDR=%s", roleKey, address))
	}
