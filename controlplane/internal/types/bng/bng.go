...
func renderBNGInstance(_ context.Context, input render.Input, cfg Config) (render.Result, error) {
	...
	dhcpHostMap, err := renderDHCPHostMap(input.Deployment, cfg)
	if err != nil {
		return render.Result{}, err
	}
	...
	return render.Result{
		...
		Artifacts: []render.Artifact{
			...
			{Key: "etc/dnsmasq.dhcp-hosts.map", Content: dhcpHostMap},
...
func renderConfigs(service string, cfg Config, devByRole map[string]string, ipByRole map[string]string) (string, string, string, error) {
	...
	for _, seg := range cfg.Access {
		if seg.DHCP4 != nil {
			v4.WriteString(dhcpLeaseCallbacks)
			break
		}
	}
...
const dhcpLeaseCallbacks = `on commit {
  execute("/etc/dhcpd-notify.sh", "commit",
    binary-to-ascii(10, 8, ".", leased-address),
    binary-to-ascii(16, 8, ":", substring(hardware, 1, 6)));
}
on release {
  execute("/etc/dhcpd-notify.sh", "release",
    binary-to-ascii(10, 8, ".", leased-address),
    binary-to-ascii(16, 8, ":", substring(hardware, 1, 6)));
}
on expiry {
  execute("/etc/dhcpd-notify.sh", "expiry",
    binary-to-ascii(10, 8, ".", leased-address),
    binary-to-ascii(16, 8, ":", substring(hardware, 1, 6)));
}
`

func renderDHCPHostMap(dep plan.Deployment, cfg Config) (string, error) {
	accessRoles := map[string]bool{}
	for _, seg := range cfg.Access {
		if seg.DHCP4 != nil {
			accessRoles[seg.Role] = true
		}
	}
	...
	for _, svc := range dep.Services {
		...
		for _, inst := range svc.Instances {
			eligible := make([]plan.Interface, 0, len(inst.Interfaces))
			primary := -1
			for _, iface := range inst.Interfaces {
				if !accessRoles[iface.Role] {
					continue
				}
				if primary == -1 || iface.DefaultRoute {
					primary = len(eligible)
				}
				eligible = append(eligible, iface)
			}
			for index, iface := range eligible {
				mac := strings.ToLower(strings.TrimSpace(iface.MAC))
				if mac == "" {
					continue
				}
				instanceAlias := fmt.Sprintf("%s-%d", svc.Name, inst.Index+1)
				aliases := []string{instanceAlias + "-" + iface.Role}
				if index == primary {
					aliases = append([]string{instanceAlias}, aliases...)
					if inst.Index == 0 {
						aliases = append([]string{svc.Name}, aliases...)
					}
				}
				for _, alias := range aliases {
					if owner, exists := aliasOwners[alias]; exists {
						return "", fmt.Errorf("duplicate DHCP DNS alias %q generated for MAC %s; already owned by MAC %s", alias, mac, owner)
					}
					aliasOwners[alias] = mac
				}
				rows = append(rows, mac+" "+strings.Join(aliases, " "))
			}
		}
	}
	if len(rows) == 0 {
...
func renderDnsmasqConf(ipByRole map[string]string, dep plan.Deployment) string {
	...
	fmt.Fprintf(&b, "addn-hosts=/etc/dnsmasq.management.hosts\naddn-hosts=/etc/dnsmasq.dhcp.hosts\n")
