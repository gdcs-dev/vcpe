...
func TestBuildImageArgs(t *testing.T) {
	...
	args, err := buildImageArgs(ImageBuildRequest{Tag: "ghcr.io/gdcs-dev/bng:dev", Context: "services/bng"})
	if err != nil {
	...
	withFile, err := buildImageArgs(ImageBuildRequest{Tag: "ghcr.io/gdcs-dev/bng:dev", Context: "services/bng", File: "services/bng/Containerfile"})
	if err != nil {
	...
	single, err := buildImageArgs(ImageBuildRequest{Tag: "ghcr.io/gdcs-dev/bng:dev", Context: "services/bng", Platforms: []string{"linux/arm64"}})
	if err != nil {
		t.Fatalf("build args single platform: %v", err)
	}
	if !reflect.DeepEqual(single, []string{"build", "--tag", "ghcr.io/gdcs-dev/bng:dev", "--platform", "linux/arm64", "services/bng"}) {
		t.Fatalf("unexpected single-platform args: %#v", single)
	}

	// Multi-platform: buildx --push
	multi, err := buildImageArgs(ImageBuildRequest{Tag: "ghcr.io/gdcs-dev/bng:dev", Context: "services/bng", Platforms: []string{"linux/amd64", "linux/arm64"}})
	if err != nil {
	...
	// No-cache
	noCache, err := buildImageArgs(ImageBuildRequest{Tag: "ghcr.io/gdcs-dev/bng:dev", Context: "services/bng", NoCache: true})
	if err != nil {
