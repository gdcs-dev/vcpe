...
func TestBuildDiagnosticJourneys(t *testing.T) {
	journeys, err := buildDiagnosticJourneys([]string{"cpe-webpa", "cpe-webpa-callback", "webhook"}, time.Second)
	if err != nil || journeys["cpe-webpa"] == nil || journeys["cpe-webpa-callback"] == nil || journeys["webhook"] == nil {
