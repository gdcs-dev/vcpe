...
Verification note: `go test ./...` is blocked only by pre-existing app-test
preconditions: `TestNamedStatusCollectsPersistedHealthOverHTTP` and
`TestNamedStatusHealthStateMatrix` cannot bind an already-used
`127.0.0.1:47000`, and `TestRunRelease_CoherenceFailure` requires the `main`
branch while this checkout is `sol-terra-high-consolidate`. Reproduce with
