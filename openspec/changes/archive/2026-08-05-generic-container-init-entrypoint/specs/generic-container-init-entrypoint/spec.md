...
The entrypoint SHALL support setting network-layer identity before DHCP runs via `VCPE_INIT_MAC_ROLE=<role>` and `VCPE_INIT_HOSTNAME=<name>`. MAC assignment SHALL use `ip link set <device> address <mac>`, where `<device>` is resolved from `IFACE_<ROLE>_DEVICE` and `<mac>` is the value of `VCPE_INIT_MAC_ROLE`. Hostname SHALL be set with the `hostname` command and SHALL be passed to DHCP as option 12.
