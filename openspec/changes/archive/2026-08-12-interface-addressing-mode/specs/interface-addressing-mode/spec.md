...
For every service type except `xb10`, the control plane's rendering and container entrypoints SHALL honor each interface's resolved `addressing` value: `dhcp` SHALL result in a real DHCP client (e.g. `udhcpc` or `dhclient`) being run against that interface's device at container start, unless the interface's network is Podman-managed (see the Podman-managed-network requirement below), in which case it is a no-op; `static` SHALL result in the interface's `ipv4`/`ipv6` being applied directly (via `ip addr add` and/or a Podman-managed `ipv4_address` compose pin, depending on the network's `ipamDriver`). `xb10` is exempt from this requirement; its existing interface initialization behavior is unchanged by this capability.
...
- **WHEN** a service interface (any type except `xb10`) resolves to `addressing: static`
