...
#### Scenario: Legacy hardcoded MAC table is not used for routerd
- **WHEN** the `routerd` container entrypoint renames interfaces at startup
- **THEN** it builds its rename table from `IFACE_*_MAC`/`IFACE_*_DEVICE` env vars and does not reference a hardcoded `LAN1_MAC`/`WAN0_MAC`/`EROUTER0_MAC` table
