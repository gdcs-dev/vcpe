...
The project SHALL provide a top-level README that documents project overview, prerequisites, build steps, run steps, and test/verification steps with executable command examples for the primary Go `vcpe` command and compatibility script paths during the migration window.
...
- **WHEN** a developer reads build and run examples after the Go operator migration
- **THEN** the examples identify `vcpe` as the primary command and describe script paths as compatibility shims
