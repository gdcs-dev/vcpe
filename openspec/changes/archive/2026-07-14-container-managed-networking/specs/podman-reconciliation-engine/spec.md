...
The `networkProvisioner` interface and `podman.EnsureNetwork` implementation SHALL accept a `NetworkSpec` struct instead of positional string parameters. When `Driver` is non-empty, `podman network create` SHALL include `--driver <driver>`. Each entry in `DriverOptions` SHALL be passed as a separate `-o key=val` flag with keys in sorted order. When `IPAMDriver` is non-empty, `--ipam-driver <driver>` SHALL be included. When `Driver` is empty the behavior SHALL be identical to the previous implementation. The `networkProvisioner` interface SHALL also provide `RemoveNetwork(ctx, name string) error` which SHALL invoke `podman network rm <name>` and return any error without forcing disconnection of attached containers. Generated `compose.yaml` service network entries SHALL omit `ipv4_address` when the interface's network has `IPAMDriver: "none"`; `mac_address` SHALL still be set so container entrypoints can deterministically rename interfaces by MAC.
