...
**Reason**: BNG rendering is no longer gated on hardcoded `bng-7`/`bng-20` smoke profiles; it is driven by typed bng-type config under the manifest-driven model.
**Migration**: Use the `Typed BNG renderer` requirement, which renders any bng-type service from typed `config.access[]`.
