## Purpose
Define how the `routerd` service type derives its config (LAN bridge topology, bridge addressing, and WAN addressing) from the manifest's existing type-agnostic fields, and how that config is rendered ahead of time and applied at container startup.
