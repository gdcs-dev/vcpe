## Purpose
Define the `vcpe stamp` developer command for pinning image tags in one or more deployment manifests before a versioned release. Stamp is a pre-release step that decouples image tag pinning from the release commit/push workflow.
