...
For every deployed service instance with health support, the system SHALL publish the container health port only on the control-plane host loopback interface and SHALL persist an endpoint record keyed by deployment name, service name, and replica index. The system SHALL allocate endpoints without collisions among active deployments and SHALL remove endpoint records when the deployment is torn down.
